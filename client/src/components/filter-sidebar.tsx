...
import { Sliders, Star, Clock, Bookmark, ChevronDown, ChevronUp } from "lucide-react";
...
export function FilterSidebar({ filters, onFiltersChange, onSelectServer, onJoinServer }: FilterSidebarProps) {
  const [pingRange, setPingRange] = useState([filters.maxPing || 150]);
  const [activeTab, setActiveTab] = useState("filters");
  const [showAdvancedFilters, setShowAdvancedFilters] = useState(false);
  const { favorites, recent, watchlist } = useFavoritesContext();
  
  // Fetch available maps from backend
  const { data: availableMaps = [] } = useQuery<string[]>({
    queryKey: ['/api/maps'],
  });
          ...
          <TabsContent value="filters" className="space-y-4 mt-4">
            ...
            <div className="space-y-2">
              ...
              <p className="text-[10px] text-muted-foreground/70 italic">
                💡 Full map data available in desktop version
              </p>
            </div>
            ...
            <Button
              variant="outline"
              className="w-full glass border-primary/30 hover:border-primary/50 transition-colors font-semibold"
            ...
            {/* AI Recommendations - Always at bottom */}
