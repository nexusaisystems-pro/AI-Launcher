...
export const ServerCard = memo(function ServerCard({ server, isSelected, onSelect, onJoin }: ServerCardProps) {
  const { isWatchlisted, addToWatchlist, removeFromWatchlist } = useFavoritesContext();
  const watchlisted = isWatchlisted(server.address);

  const handleCardClick = () => {
    onSelect();
    // Scrolling is now handled by virtualizer.scrollToIndex in ServerBrowser for better performance
  };
