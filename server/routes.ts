...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/auth/user', isAuthenticated, async (req: any, res) => {
    try {
      ...
      // If user exists in DB, use their DB role. Otherwise, use OIDC claims role for testing
      if (user) {
        res.json(user);
      } else if (req.user.claims.role) {
        // User not in DB but has OIDC role claim (testing scenario)
        res.json({
          id: userId,
  ...
  app.get("/api/servers", async (req, res) => {
    try {
      ...
      if (page !== undefined || limit !== undefined) {
        const { servers: paginatedServers, total } = await storage.getServersPaginated(
          validatedFilters, 
          page || 1, 
          limit || 50,
          sortBy || 'players'
      ...
      // Legacy: return all servers if no pagination params
  ...
  // Get sponsored servers (MUST be before :address route)
  app.get("/api/servers/sponsored", async (req, res) => {
    try {
      const includeIntelligence = req.query.intelligence === 'true';
      const limit = req.query.limit ? parseInt(req.query.limit as string) : 3;
      const sponsoredServers = await storage.getSponsoredServers(limit);
      ...
      res.json(sponsoredServers);
    } catch (error) {
      console.error("Error fetching sponsored servers:", error);
      res.status(500).json({ error: "Failed to fetch sponsored servers" });
    }
  });

  // Desktop launcher API - Get all servers in DZSA format
  app.get("/api/v2/launcher/servers/dayz", async (req, res) => {
    try {
      // Get all servers with mod data
      const { servers } = await storage.getServersPaginated(undefined, 1, 10000);
      
      // Format in DZSA-compatible structure
      const dzsaFormat = servers.map(server => {
        ...
        return {
          ...
          battlEye: true, // Assume BattlEye enabled
          firstPersonOnly: server.perspective === '1PP',
          shard: 'private',
          timeAcceleration: 1,
          time: "12:00",
          mods: server.modList?.map(mod => ({
      ...
      console.error("Error fetching servers for desktop launcher:", error);
  ...
  // Database-first search for instant results
  app.get("/api/servers/search/realtime", async (req, res) => {
    try {
      ...
      console.log(`[API] Database search for: "${query}"`);
      
      // Search database first for instant results
      const dbServers = await storage.searchServersByName(query.trim(), 100);
      
      // Enrich with cached BattleMetrics data and intelligence scores
      const enrichedServers = await Promise.all(
        dbServers.map(async (server) => {
          const bmCache = await storage.getBattleMetricsCache(server.id);
          const quality = serverIntelligence.calculateQualityScore(server, bmCache);
      ...
      console.error('[API] Database search failed:', error);
    ...
    try {
      ...
      const modsWithMetadata = await Promise.all(
        (server.mods || []).map(async (mod) => {
          if (mod.workshopId) {
            try {
              const workshopDataArray = await steamWorkshopService.getWorkshopItemDetails([mod.workshopId]);
              const workshopData = workshopDataArray[0];
              return {
                ...mod,
                workshop: workshopData ? {
                  title: workshopData.title,
                  author: workshopData.creator,
                  subscriberCount: workshopData.subscriberCount,
                  fileSize: workshopData.fileSize,
                  lastUpdated: workshopData.timeUpdated,
                  previewUrl: workshopData.previewUrl,
                } : null
              };
            } catch (error) {
  ...
  app.get("/api/maps", async (req, res) => {
    try {
      const maps = await storage.getAvailableMaps();
      res.json(maps);
    } catch (error) {
      res.status(500).json({ error: "Failed to fetch available maps" });
    }
  });
  ...
  app.get("/api/admin/claims", isAuthenticated, isAdmin, async (req: any, res) => {
    try {
      const claims = await storage.getAllPendingClaims();
      res.json(claims);
    } catch (error) {
  ...
  app.put("/api/admin/claims/:id/approve", isAuthenticated, isAdmin, async (req: any, res) => {
    try {
      ...
      const claim = await storage.approveClaim(id, adminId);
      
      if (!claim) {
        return res.status(404).json({ error: "Claim not found" });
      }
      
      // Create server owner record
      await storage.createServerOwner({
        serverAddress: claim.serverAddress,
        userId: claim.userId,
        ownerEmail: "", // Will be fetched from user
        ownerSessionId: claim.userId, // Use userId as fallback
  ...
  app.put("/api/admin/claims/:id/reject", isAuthenticated, isAdmin, async (req: any, res) => {
    try {
      ...
      const claim = await storage.rejectClaim(id, adminId, reason);
  ...
  // Get all servers (for admin monitoring)
  app.get("/api/admin/servers", isAuthenticated, isAdmin, async (req: any, res) => {
    try {
  ...
  // ============================================
  // BULK ENRICHMENT ROUTES (Requires admin role)
  // ============================================

  // Start bulk enrichment of all servers
  app.post("/api/admin/enrich-servers", isAuthenticated, isAdmin, async (req: any, res) => {
    try {
      const progress = bulkEnrichmentService.getProgress();
      
      if (progress.isRunning) {
        return res.status(409).json({ 
          error: "Enrichment is already in progress",
          progress 
        });
      }

      // Start enrichment in background (don't await)
      bulkEnrichmentService.enrichAllServers(true).catch(error => {
        console.error("[API] Bulk enrichment failed:", error);
      });

      res.json({ 
        message: "Bulk enrichment started successfully",
        progress: bulkEnrichmentService.getProgress()
      });
    } catch (error) {
  ...
  app.get("/api/admin/enrich-servers/progress", isAuthenticated, isAdmin, async (req: any, res) => {
    try {
      const progress = bulkEnrichmentService.getProgress();
      res.json(progress);
    } catch (error) {
      console.error("Error fetching enrichment progress:", error);
      res.status(500).json({ error: "Failed to fetch enrichment progress" });
    }
  });

  // Reset enrichment progress (for testing/debugging)
  app.delete("/api/admin/enrich-servers/progress", isAuthenticated, isAdmin, async (req: any, res) => {
    try {
      const progress = bulkEnrichmentService.getProgress();
      
      if (progress.isRunning) {
        return res.status(409).json({ 
          error: "Cannot reset progress while enrichment is running"
        });
      }

      bulkEnrichmentService.resetProgress();
